...
| 6 | `a9176189` | install succeeds; merge-with-main resolves cleanly | Merge commit pulling in main; conflict in `studio/install_llama_prebuilt.py` resolved by keeping the feature-branch per-target patterns (strict superset of upstream's simplified glob). Lemonade install path identical to pass 5: same bundle b1280, same 80 `.so` files, 0 missing libs, summary line + metadata intact. |
...
| **Pass 6 (2026-05-27 late)** | |
| `install-a9176189-verbose.log` | Verbose run after Leo's merge-with-main commit; identical successful flow to pass 5 (lemonade b1280 selected, validated, activated) |
| `UNSLOTH_PREBUILT_INFO-a9176189.json` | Install metadata snapshot — `binary_repo: lemonade-sdk/llamacpp-rocm`, `binary_release_tag: b1280`, `source: lemonade` |
| `ldd-llama-server-a9176189.txt` | Full transitive resolution from activated `llama-server` — 0 missing libs |
